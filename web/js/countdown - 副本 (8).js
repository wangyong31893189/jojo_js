...
this.options = {
	...
	recoverDutchActions:function(dutchActions){
		...
		//销售开始时间
		var sellStartTime=dutchActions.sellStartTime;
		//服务器当前时间
		var currentTime=dutchActions.currentTime;
		//销售结束时间
		var sellEndTime=dutchActions.sellEndTime;
		//商品价格
		...
		//当前时间   转换为date
		var current=this.parseDate(currentTime);
		//结束时间   转换为date
		...
		for(var i=1;i<times-1;i++){
			...
			downRule.currentDate=current;			
			downRule.endTime=nextDate.format("yyyyMMddhhmmss");
			downRule.endDate=nextDate;
			downRule.currentTime=dutchActions.currentTime;			
			//计算出每次降价过程中的售价
