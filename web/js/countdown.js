...
this.options = {
    ...
    //结束时间点时间对象
    //endStemp : null,
	ssjcountdownobj:null,
	//开时时间对象
	//beginStemp:null,
	ssjcountindex:0,
	
	ssjcountpreprice:0,//记录上一次的售价
	//输入的服务器当前时间对象
	currStemp:null,
	//销售开始倒计时时间结束点 以ms为单位
    ...
    init : function(o){
		...
		this.dutchNextDate=this.sellBeginTimeStemp;
		...
		if(this.dataOptions){
			if(this.dataOptions.dutchActions){
				...
				this.ssjcountpreprice=this.dataOptions.dutchActions.price;//记录当前的价格
		...
		if(this.countDownBeginStemp<= 0){
			...
			this.ssjcountindex=parseInt((this.currStemp.getTime()-this.sellBeginTimeStemp.getTime())/(this.dataOptions.dutchActions.downTimes*1000));
			var moreSec=(this.currStemp.getTime()-this.sellBeginTimeStemp.getTime())%(this.dataOptions.dutchActions.downTimes*1000);
			//if(moreSec>0){
				this.dutchNextDate=new Date(this.sellBeginTimeStemp.getTime()+(this.ssjcountindex*this.dataOptions.dutchActions.downTimes*1000));//如果已经开始了 那下一次降价开始时间应该等于当前时间
				this.ssjcountpreprice=this.dataOptions.dutchActions.price-this.ssjcountindex*this.dataOptions.dutchActions.everyDownPrice;
				//console.info("当前的价格为："+this.ssjcountpreprice);
			//}
			//if(this.ssjcountindex==0){
			//	this.ssjcountindex=0;
			//}
			//console.log("已经降了"+this.ssjcountindex+"次了！");
        }
	...
	/*recoverDutchActions:function(dutchActions){
		//{'downTimes':'10','finalPrice':'30.0','everyDownPrice':'5','price':'99.00'}},
		//限购数量
		...
		for(var i=1;i<times;i++){
			var downRule={};
			//下一次时间
			var nextDate=new Date(sellStartTime.getTime()+(downTimes*1000));
			//设置每次降价的起始时间和终止时间
			if(sellStartTime.getTime()>sellEndTime.getTime()){
				sellStartTime=sellEndTime;
			}
			//设置索引
			downRule.index=i;
			downRule.startTime=sellStartTime.format("yyyyMMddhhmmss");
			downRule.startDate=sellStartTime;			
			...
			sellStartTime=nextDate;
			lastPrice=downRule.price;
			downRule=this.covertMdseRuleToJsonStr(downRule);
			if(downRule){
				if(parseFloat(downRule.cutPrice)>0){
		...
		var logTemp="[";
		for(var i in ssjDownList){
			logTemp+=JSON.stringify(ssjDownList[i]);
		}
		console.log("输出json字符："+logTemp+"]");
		//console.log(ssjDownList);
		this.dataOptions.dutchActions=ssjDownList;
	},*/
	//降价倒计时初始化  新的规则解析
	getRecoverDutchActions:function(dutchActions){
		//{'downTimes':'10','finalPrice':'30.0','everyDownPrice':'5','price':'99.00'}},
		//限购数量
		var limitBuy=dutchActions.limitBuy;
		//降价频率  每隔多少秒降一次
		...
		var sellStartTime=this.dutchNextDate;
		...
		//for(var i=1;i<times;i++){
			var downRule={};
			//下一次时间
			var nextDate=new Date(this.dutchNextDate.getTime()+(downTimes*1000));
			this.dutchNextDate=nextDate;
			//设置每次降价的起始时间和终止时间
			if(sellStartTime.getTime()>sellEndTime.getTime()){
				sellStartTime=sellEndTime;
			}
			var i=this.ssjcountindex;
			//console.log("还剩下"+(times-i)+"次需要减");
			if((times-i)<=0){
				//console.log("不用再倒计时，清除倒计时！"); 
				return null;
			}
			//设置索引
			downRule.index=i;
			...
			downRule.endTime=nextDate.format("yyyyMMddhhmmss");
			downRule.endDate=nextDate;
			//console.log("-----------downRule.endDate="+downRule.endDate);
			downRule.currentTime=currentTime;			
			//计算出每次降价过程中的售价
			var old=new Number(price-(everyDownPrice*i)).toFixed(2);
			//如果每次降价过程中的售价小于该商品规定的最终售价，以最终售价为准
			if (old< finalPrice) {
				//everyDownPrice = everyDownPrice-(finalPrice-old);
				old = finalPrice;
			...
			downRule.prevPrice=old-everyDownPrice;//记录上次的的价格;			
			sellStartTime=nextDate;
			lastPrice=downRule.price;
			downRule=this.covertMdseRuleToJsonStr(downRule);
			this.ssjcountindex++;
	...
	covertMdseRuleToJsonStr:function(downRule){
		...
		if (downTime) {				
				...
				var currentDate=this.currStemp;
				var endDate=downRule.endDate;
				//var startDate=this.sellBeginTimeStemp;
				//var currentDate=this.currStemp;
				//var endDate=this.sellEndTimeStemp;
				//calender.setTime(format.parse(endTime));
				var startTimeInMillis = startDate.getTime();
				var currentTimeInMillisTemp = currentDate.getTime();
				var endTimeInMillis = endDate.getTime();
				
				//**判断如果当前规则已经在倒计时，那么要用结束时间 减去当前时间，获得还没有倒计时的毫秒数*//*
				if(currentTimeInMillisTemp>=startTimeInMillis&& currentTimeInMillisTemp<=endTimeInMillis){					
					downTimeInMillis = endTimeInMillis-currentTimeInMillisTemp;
					//console.log("----------------------------倒计时时间："+downTimeInMillis);
					downRule.endTimeStemp=downTimeInMillis;//毫秒
						...
						downRule.cutPrice=new Number(downRule.disCountPrice).toFixed(2);
						downRule.endPrice=new Number(downRule.price).toFixed(2);
						downRule.prevPrice=new Number(downRule.prevPrice).toFixed(2);	
						//console.log("倒计时时间："+downTimeInMillis);
						//console.log("当前价格："+downRule.endPrice+"，商品降价开售时间："+downRule.start+"，商品结束时间"+downRule.end+"，优惠价格："+downRule.cutPrice+"，上一次的售价"+downRule.prevPrice);
				}else{
					downTimeInMillis = downRule.downTimes*1000;
				
					if(currentTimeInMillisTemp<=startTimeInMillis&&currentTimeInMillisTemp<=endTimeInMillis){//只处理在有效的，当前和以后的规则
							//downRule.index= i+1);
							//downRule={};
				...
				if(downRule){
					//downRule.cutPrice=new Number(downRule.prevPrice-downRule.endPrice).toFixed(2);
					console.log("当前价格："+downRule.endPrice+"，商品降价开售时间："+downRule.start+"，商品结束时间"+downRule.end+"，优惠价格："+downRule.cutPrice+"，上一次的售价"+downRule.prevPrice+",当前的结束时间点="+downRule.endTimeStemp);
				}								
				return downRule;
		}
	},
	...
	**/
	_renderDutchAuctionTime:function(){		
		...
		if(this.isSellBeginEnd===true){
			...
			if(datas!=null){				
				...
				if(this.index==true||this.index==0){
				//console.log("------------this.index="+this.index);
					this.ssjcountdownobj=this.getRecoverDutchActions(datas.dutchActions);	
					if(this.ssjcountdownobj){
						this.ssjcountpreprice=this.ssjcountdownobj.endPrice;//记录上次的的价格
						//console.log("当前价格是多少endPrice="+this.ssjcountdownobj.endPrice+"----prevPrice="+this.ssjcountdownobj.prevPrice);
					}
				}		
				var obj=this.ssjcountdownobj;
				if(this.ssjcountdownobj){
					window.clearInterval(that.sellEndtimer);
					//if(this.dutchActionTimeStamp){
						this.dutchActionTimeStamp=obj.endTimeStemp;
						//console.log("时时降时间计时开始！this.dutchActionTimeStamp="+this.dutchActionTimeStamp);					
							//window.setTimeout
				...
				window.clearInterval(that.sellEndtimer);
				//console.log("22222this.dataOptions.dutchActions.length已经完全结束了！");
				if(this.isDutchEnd===false){//未完全结束
					if(datas==null){
						var _dutchAuctionTime=document.getElementById("dutchAuctionTime"+this.countId);
						_dutchAuctionTime.innerHTML="<font class=\"size18 cor03\">最低价啦</font>";
    ...
    _renderDutchAuctionTimeShow : function(obj){	
		...
		var t =this.dutchActionTimeStamp;
			// 总秒数  
		var xt = parseInt(t/1000)-1;			
		//console.log("销售进行中》》》XT==="+xt);		
		this.dutchActionTimeStamp=this.dutchActionTimeStamp-1000;				
		//if(this.isSellEndEnd===true){
		this.index=false;
		if(this.isDutchEnd===false){	//未完全结束	
			if(parseFloat(this.dataOptions.lowestPrice)>=parseFloat(obj.endPrice)){				
				var _dutchAuctionTime=document.getElementById("dutchAuctionTime"+this.countId);
				_dutchAuctionTime.innerHTML="<font class=\"size18 cor03\">最低价啦</font>";
				var _nowTotalPrice=document.getElementById("nowTotalPrice"+this.countId);
				_nowTotalPrice.innerHTML="现价￥"+this.dataOptions.lowestPrice+"元";
				...
				if(xt>=0){					
					// console.log(_showObj.innerHTML);		
					_dutchAuctionTime.innerHTML="<em><span></span>"+(xt+1)+"</em><font>秒后</font><i><img src=\""+this.projectImageUrl+"ssj05.jpg\" width=\"15\" height=\"16\"></i><em><span></span>"+obj.cutPrice+"</em><font>元</font>";
					//if(xt==0){
					//	this.index=true;//设置为true 说明可以进行下一次的倒计时
					//	_nowTotalPrice.innerHTML="现价￥"+obj.endPrice+"元";
					//	console.log("xt="+xt+"当前降价的前价="+obj.prevPrice+",后价是="+obj.endPrice);
					//}else{
						this.index=false; //设置为false 说明还在这一次的倒计时
						_nowTotalPrice.innerHTML="现价￥"+obj.endPrice+"元"; 
						//console.log("xt="+xt+"当前降价的前价="+obj.endPrice+",后价是="+obj.endPrice);
					//}
					/*if(this.index>0){						
